...
def test_default_policy_is_complete_and_uses_exact_rates() -> None:
    ...
    assert POLICY_V1.instructor_prompt_version == "instructor_v1"
    assert POLICY_V1.item_writer_prompt_version == "item_writer_v1"
...
def test_policy_requires_flow_specific_versioned_prompt_ids() -> None:
    with pytest.raises(InvalidPolicy):
        replace(POLICY_V1, instructor_prompt_version="item_writer_v1")
